...
for (let i = 0; i < (video.scenes || []).length; i += 1) {
  ...
  cumStart += dur;
  // Screencast beats: kind:screen, or the consolidated kind:broll fit:'bleed'
  // full-bleed screen recording (the vektor screencast treatment, 2026-07-18).
  const isScreencast = scene.kind === 'screen' || (scene.kind === 'broll' && scene.fit === 'bleed');
  if (!isScreencast) continue;
  if (!scene.src) { console.error(`scenes[${i}]: screencast scene has no src — skipping`); continue; }
  ...
  const prompt = [
    'You are an AI video editor deciding how to treat a screen recording that plays FULL-BLEED in a vertical (portrait) short-form video.',
    `Read these ${FRAMES} frames IN ORDER from ${tmp} (each labelled with its scene-frame, 0..${dur} @${fps}fps):`,
    frameList,
    '',
    narrBlock,
    '',
    'FIRST, make an EDITORIAL DECISION about this beat:',
    '- If it is a DETAIL WALKTHROUGH — specific things on screen matter and the viewer should be guided to them (a command, an output, a line of the plan the narration names) — do a DYNAMIC cinematic zoom: push into those regions, follow them, pull back.',
    '- If it is an OVERVIEW / AMBIENT beat — the vibe or the whole screen matters more than any one spot, or nothing specific is being pointed at — keep it CALM: reply with exactly the single word CALM (no JSON). The engine will apply a gentle, steady drift instead.',
    '',
    'IF you chose DYNAMIC, output ONLY a JSON array of 5-7 keyframes, smooth arc, first at:0 and last at:' + dur + ':',
    '[{"at":0,"scale":1.0,"xPct":50,"yPct":50}, ...]',
    'with these COORDINATE CONSTRAINTS (full-bleed portrait crops the left/right edges of a landscape source):',
    ...
    'No prose, no code fences — just the JSON array. OR, if CALM, just the word CALM.',
  ].join('\n');

  process.stderr.write(`\n  scenes[${i}] ${scene.kind} · ${scene.src} · ${dur}f · narration:${narration ? 'yes' : 'none'} — asking claude...\n`);
  ...
  // The AI's editorial call: CALM (no JSON) → leave the default gentle drift
  // (clear any prior keyframes); else a dynamic keyframe array.
  const out = (res.stdout || '').trim();
  if (/^CALM\b/i.test(out) || (!out.includes('[') && /\bCALM\b/i.test(out))) {
    if (scene.zooms) delete scene.zooms;
    console.log(`\n  scenes[${i}] → CALM (gentle drift, no keyframe zoom)`);
    changed += 1;
    continue;
  }

  let zooms;
  try { zooms = validateZooms(extractZooms(out), dur); }
  catch (e) { console.error(`  could not parse zooms: ${e.message}\n  ---\n${out.slice(0, 500)}`); continue; }

  console.log(`\n  scenes[${i}] → DYNAMIC zoom (${zooms.length} keyframes):`);
