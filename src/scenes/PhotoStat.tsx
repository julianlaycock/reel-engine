...
export const PhotoStat: React.FC<{scene: any; hideChrome?: boolean}> = ({scene, hideChrome}) => {
  ...
  return (
    <AbsoluteFill style={{background: 'var(--bg-mid, #E7E0D0)', overflow: 'hidden'}}>
      ...
      <div style={{position: 'absolute', top: 340, bottom: 580, [side]: 44, width: '54%', transform: `rotate(${side === 'right' ? -2.4 : 2.4}deg) scale(${z * 0.94})`, transformOrigin: 'center', border: '2px solid rgba(16,16,16,0.55)', boxShadow: `${side === 'right' ? -16 : 16}px 20px 0 rgba(16,16,16,0.32)`, overflow: 'hidden', background: '#fff'}}>
      ...
      <div style={{position: 'absolute', top: 300, [side === 'right' ? 'left' : 'right']: 70, maxWidth: 560}}>
        ...
        <div style={{fontFamily: '"Anton", sans-serif', fontSize: 200, lineHeight: 0.84, color: 'var(--accent)', transform: `scale(${statScale})`, transformOrigin: `${side === 'right' ? '0%' : '100%'} 50%`, opacity: statOp, letterSpacing: '-0.02em'}}>
          {lines.map((l, i) => <div key={i}>{l}</div>)}
        </div>
        {scene.statSub ? (
          <div style={{fontFamily: 'var(--display)', fontSize: 40, fontWeight: 800, color: 'var(--fg)', marginTop: 16, opacity: capRise, transform: `translateY(${(1 - capRise) * 14}px)`, maxWidth: 460, lineHeight: 1.05}}>{scene.statSub}</div>
        ) : null}
        {scene.caption ? (
          <div style={{fontFamily: 'var(--mono)', fontSize: 26, color: 'var(--muted)', marginTop: 14, opacity: capRise, letterSpacing: '0.02em'}}>{scene.caption}</div>
        ) : null}
      </div>
    </AbsoluteFill>
  );
