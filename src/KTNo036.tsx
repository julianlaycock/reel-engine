...
import {CountGrid} from './kt/argument';
...
// ═══ S1 — THE SESSION GRID ═══════════════════════════════════════════════════
// The re-reading, drawn: a grid of files fills while the voice says "the same
// greps, the same file reads". DELIBERATELY NO NUMBER on this plate — the grid
// is abstract (5x10) and its caption names the behaviour, not a count. Drawing
// "196" on 50 cells would be the drawn-evidence mistake of 2026-08-14; the real
// 196 belongs to beat 3, where the capture that owns it is on screen.
const GRID_BUILD_FROM = 3920;   // "Every new session"
const GRID_BUILD_TO = 9090;     // "...the same file reads." — the last cell lands
const SessionGrid: React.FC<{field: string}> = ({field}) => (
  <CountGrid field={field} cols={10} rows={5}
    buildFromMs={GRID_BUILD_FROM} buildToMs={GRID_BUILD_TO}
    caption="EVERY SESSION, FROM SCRATCH" />
);
...
const PLATES: {from: number; to: number; Node: React.FC<{field: string}>}[] = [
  {from: 120,   to: 13360,        Node: SessionGrid},   // 13.2s  4 words  needs 5.4
  {from: 36320, to: 40000,        Node: QueryTerm},     //  3.7s  ~12 mono words — see check
  {from: 40000, to: 50560,        Node: TokenBars},     // 10.6s  8 words  needs 7.8
  {from: 50560, to: 61760,        Node: HonestyPlate},  // 11.2s 14 words  needs 11.4 — tight; struck line leaves early
